 ...
 *   4) 특수문자(_,$) 사용O => _ 사용多
 *   	*** _클래스명: 임시클래스
 *   	약속사항: 대문자로 시작(헝거리언식 표현) FileName
 *   			--- 
 *   			객체명은 변수명과 동일하게 만듦
 *   			----	--- (일반변수)
 *   			클래스의 변수명
 ...
 *   	------------
 *   	class music;
 *   	music music; ==> 클래스명과 객체명 구분 못함
 *   
 *    => 메모리 저장
 *    	=========
 *    	Music m=new Music();
 *    				------- 생성자 => 생성자는 반드시 앞에 new 붙여서 사용해야 함 
 *    	클래스의 구성요소
 *    	---- 재사용 기법에서 많이 쓰는 형태 (is-a,has-a)
 *    	1. 변수 ====> 캡슐화
 *    	2. 메소드 ===> 수정/추가 => 다형성
 *    	3. 생성자 ===> 필요할 경우에만 사용
 *    	--------------- 객체지향 형식으로 변형 
...
public class 객체지향프로그램_4 {
	...
	public static void main(String[] args) {
		...
		Music[] music=new Music[200]; //클래스는 일반변수와 동일 (일반변수처럼 배열 사용이 가능)
		// 필요할 때만 데이터 여러개를 모아 관리 : 클래스 ==> 클래스를 메모리에 저장: 객체(인스턴스) 
		try
		{
			int k=0;
			for(int i=1;i<=4;i++)
			{
				...
				// HTML -> tag(<>)로 이루어짐 => 자바 -> Element
        		org.jsoup.select.Elements title=doc.select("tr.list a.title");
        		org.jsoup.select.Elements singer=doc.select("tr.list a.artist");
        		org.jsoup.select.Elements album=doc.select("tr.list a.albumtitle");
        		org.jsoup.select.Elements etc=doc.select("tr.list span.rank");
        		org.jsoup.select.Elements poster=doc.select("tr.list a.cover img");
