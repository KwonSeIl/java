 ...
 * 					==> 컴포넌트
 * 				------------------
 * 				컴포넌트(클래스 -> 한개의 전체 기능을 가지고 있다) -> 조립 -> 사이트 완성
 * 				==> 객체지향의 특성을 첨부 -> 유지보수가 편하게 만든다 . 문법사항X 권장사항O
 * 					----------
 * 					1)캡슐화: 데이터 보호 => 데이터를 은닉화, 메소드를 통해서 접근하는 방식(메소드 통해 변수 사용)
 * 									 -----------	-----------------
 * 										private		읽기/쓰기 => getter/setter
 * 					2)상속/포함: 재사용(소스 중복 제거)
 * 					3)다형성: 재사용 필요시에는 언제든지 수정, 기능 추가
 * 										(오버라이딩), (오버로딩)
 * 				==> 클래스의 종류
 * 					1) 추상클래스
 * 					***2) 인터페이스: 독립적으로 사용 가능, 서로 다른 클래스를 연결해서 사용(리모콘 역할)
 * 							결합성이 낮은 프로그램 가능
 * 							---- 클래스에 미치는 영향(클래스를 수정시에 다른 클래스에 영향) => 스프링(결합성 낮음)
 * 							응집성
 * 							---- 메소드 관련(한가지 기능) -> 재사용 가능 
 * 					3) 내부클래스: 쓰레드,윈도우에서 주로 사용 => 하둡(빅데이터)
 * 						- 익명의 클래스: 상속 없이 오버라이딩
 * 
 * 					*** 추상클래스 / 인터페이스는 메모리 할당이 불가능하다 -> 반드시 상속을 받아서 구현된 클래스를 이용해야 함
 * 						인터페티스는 추상클래스의 일종 => 추상클래스의 단점을 보완
 * 						사용방법
 * 						  1)상속: 추상클래스는 단일 상속만 가능(extends)
 ...
 * 									*** 프로그램: 여러개를 그냥 사용하면 나중에 수정하기 어려울 수 있음
 * 												--- 한개의 이름으로 제어
 * 												데이터 => 배열명
 * 												클래스 여러개 => 인터페이스 이용해 제어
 * 						  2)메모리 할당
 * 							class Human
 * 							{
 * 								public void display(){}
 ...
 * 							3) Human h3=Human.class.getDeclearedContstructor().newInstnce()
 ...
 * 						  4)클래스: 클래스와 클래스의 연관관계(객체지향) => public
 * 							메소드: 클래스와 클래스의 통신을 하는 기능 => public
 * 							변수: 데이터 보호 목적 => private
 * 							생성자: 클래스의 기능을 얻어올 때 사용(다른 클래스에 메모리 할당) => public
 * 		***원하는만큼 저장: 컬렉션(11~12장): **List, Set(중복된 데이터를 제거), **Map(코테) => 클래스 관리
 * 									  ------ DB						---- => 웹에서 지원하는 모든 클래스
 * 																	 Request, Response
 * 																	 Cookie
 * 			*** 12장(어노테이션) => 스프링은 전체 다 어노테이션으로 이뤄짐
 * 			
 * 		------------------ 메모리 저장
 * 		파일(15장): 파일 입출력=> File/FileReader/FileWriter/FileInputStream/FileOutputStream
 * 							ObjectInputStream/ObjectOutputStream/BufferdReader/BufferdWriter
 * 		***RDBMS(오라클): DML/DDL/DCL/TCL => SQL
 * 						---JOIN/SBUQuery/Index/View(인라인뷰)
 * 						기본) DB모델링
 * 						-----------JDBC(핵심) => 자바로 오라클 연결
 ...
 *  				= 반복제어문(break)
 *  			 메소드
 *  				형식) [접근지정어][제어어] 리턴형 메소드명(매개변수..) 선언부
 *  					{
 *  						구현부
 *  					}	
 *  					[접근지정어] => 접근지정어 전체 올 수 있음(public,default,protected,private)
 *  													-------
 *  					[제어어] => abstract, final, static
 *  
 *  					=> void 외에는 반드시 return을 사용해야 한다(return: 메소드 종료)
 *  					=> 리턴형: 요청 처리에 대한 결과값
 *  							리턴형은 한개만 설정이 가능(여러개 데이터를 전송)
 *  							------------------- 클래스 전송, 배열 전송
 *  					=> 매개변수: 사용자 요청값(여러개 나올 수 있음)
 *  					=> 매개변수 전송법: Call By Reference / Call By Value
 *  									주소 전송(배열,클래스)	실제값 전송(새로운 메모리에 값만 전송)
 *  														기본형 / String
 *  3. 데이터 출력: 브라우저 출력(웹 프로그램) => HTML/CSS만 출력 가능(브라우저 안에서 자바는 일반 텍스트)
