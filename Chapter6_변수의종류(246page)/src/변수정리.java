 ...
 * 		-------	------------	-----(캡슐화)
 * 		*** 캡슐화, 오버라이딩, 포함
 * 	 class JFrame: 1000줄 이상 코딩
 * 	 class A extends JFrame -> 코딩하지 않고 가져올 수 있음
 * 	객체 지향 제작
 * 	----------
 ...
 * 				 메소드만 가지고 있는 클래스: 액션 클래스(기능만 가지고 있음)
 * 				 데이터+메소드 가지고 있는 클래스: 조립 클래스(main())
 * 		클래스 메모리 저장
 * 			클래스명 변수명=new 클래스명()
 * 			---------------------- new 연산자는 새로운 메모리를 만들어줌 
 * 
 * 		1) 변수
 * 			인스턴스 변수: new를 이용할 때마다 따로 생성되는 변수(값 저장 시 다른 데이터값을 가지고 있음) **** 주로 사용되는 변수
 * 			클래스 변수: 컴파일러에 의해 자동으로 생성되는 변수=> 메모리 공간이 한개이기 때문에 모든 데이터값이 동일 
 * 						공유할 때 사용
 * 						static 사용(네트워크) => 서버(접속자 정보) => IP
 * 			=> 모든 데이터가 static: Oracle
