 ...
 * 						 실제로 오버라이딩(변경) -> String에서 실제 저장된 문자열만 비교 
 * 			   4) 문자열 변환 => toString(): 생략할 수 있음(객체명만 출력) 
 ...
 * 				String클래스에 유일하게 static이다
 * 				String.valueOf()
 * 		= StringBuffer: 문자열 결합 시 많이 사용되는 클래스
 * 			문자열 결합: append()
 * 				원형) public StringBuffer append(String s) => 오버로딩 -> double, int ...
 * 			문자열 변환: toString()
 * 				원형) public String toString()
 * 					(String)
 * 		= Wrapper: 기본형 데이터형을 쉽게 사용할 수 있게 클래스형으로 제작(모든 데이터형이 클래스화)
 * 			= 제네릭스 사용, Object로 변환할 때 많이 사용 
 ...
 * 			int ii=i;
 * 		= System: gc()
 * 		= Math: random():난수 발생, ceil():올림, round():반올림
 * 	java.util
 * 		= StringTokenizer
 * 		= Scanner
 * 	java.util.regex
...
public class 정리_1권_라이브러리 {
	...
	public static void main(String[] args) {
		...
		A a=new A(); 
		//String s=a.toString(); //String s=a 오류
		//System.out.println(a); //.toString 생략 => 출력시에만 가능
